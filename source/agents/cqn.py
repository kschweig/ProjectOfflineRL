...
class CQN(Agent):
    ...
    def __init__(self, params):
        ...
        super(CQN, self).__init__(params.action_space, params.frame_stack)
    ...
    def train(self, replay_buffer):
        ...
        with torch.no_grad():
            target_Q, pred_next_state = self.Q_target(next_state)

            uncertainty = torch.mean(((pred_next_state - next_state)**2).reshape(self.params.batch_size, -1), dim=1)
            # bring to reasonable range in [0,1]:
            certainty = 1 - torch.clip(uncertainty/(next_state.mean().item()**2), 0, 1).unsqueeze(1)
            target_Q = reward + done * certainty * self.params.discount * target_Q.max(1, keepdim=True)[0]

        # Get current Q estimate
        current_Q, pred_state = self.Q(state)
        certainty_loss = torch.mean(((pred_state - state)**2).reshape(self.params.batch_size, -1))
        current_Q = current_Q.gather(1, action)

        # Compute Q loss (Huber loss)
        Q_loss = self.huber(current_Q, target_Q)
        Q_loss += certainty_loss
...
class Network(nn.Module):
    ...
    def __init__(self, frames, num_actions, duelling=False):
        ...
        self.deconv = nn.Sequential(
            ...
            nn.SELU(),
            nn.ConvTranspose2d(in_channels=32, out_channels=frames, kernel_size=8, stride=4)
        )
    ...
    def forward(self, state):
        ...
        # if not, simple calculate values via advantage stream
            ...
            return self.advantage_stream(features), self.deconv(features)
